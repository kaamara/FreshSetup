...
2. W kolumnie **Pliki i zakładki** dodaj pliki, foldery i linki oraz wskaż plik zakładek Brave (`brave://bookmarks` → ⋮ → Eksportuj zakładki).
3. Zamknij aplikację i skopiuj cały folder `FreshSetup` (razem z `payload\`) na pendrive lub do chmury.

### 2. Na nowym komputerze

1. Skopiuj folder na dysk i uruchom `FreshSetup.bat`.
2. Zaznacz interesujące pozycje.
3. Kliknij **Start**.
